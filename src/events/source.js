const data = {
	"id": 3,
	"name": "Concerts",
	"events": [],
	"children": [
		{
			"id": 1207,
			"name": "Rock and Pop",
			"events": [],
			"children": [
				{
					"id": 429,
					"name": "Backstreet Boys",
					"children": [],
					"events": [
						{
							"id": 4422829,
							"name": "Backstreet Boys",
							"venueName": "Freedom Mortgage Pavilion",
							"city": "Camden",
							"price": 23,
							"distanceFromVenue": 121162.390908814,
							"date": "Jul 16 2020"
						},
						{
							"id": 4422826,
							"name": "Backstreet Boys",
							"venueName": "Xfinity Theatre",
							"city": "Hartford",
							"price": 20,
							"distanceFromValue": 160356.993027791,
							"date": "Jul 17 2022"
						},
						{
							"id": 150029205,
							"name": "Backstreet Boys",
							"venueName": "Bethel Woods Center for the Arts",
							"city": "Bethel",
							"price": 69,
							"distanceFromValue": 122298.256317016,
							"date": "Jul 24 2022"
						}
					]
				},
				{
					"id": 450948,
					"name": "Justin Beiber",
					"children": [],
					"events": [
						{
							"id": 8621837,
							"name": "Justin Beiber",
							"venueName": "Freedom Mortgage Pavilion",
							"city": "Camden",
							"price": 23,
							"distanceFromVenue": 121162.390908814,
							"date": "Jul 16 2020"
						},
						{
							"id": 8621843,
							"name": "Justin Beiber",
							"venueName": "Xfinity Theatre",
							"city": "Hartford",
							"price": 20,
							"distanceFromValue": 160356.993027791,
							"date": "Jul 17 2022"
						},
						{
							"id": 8621842,
							"name": "Justin Beiber",
							"venueName": "Bethel Woods Center for the Arts",
							"city": "Bethel",
							"price": 69,
							"distanceFromValue": 122298.256317016,
							"date": "Jul 24 2022"
						}
					]
				}
			]
		},
		{
			"id": 1208,
			"name": "R&B/Urban Soul",
			"events": [],
			"children": [
				{
					"id": 106817,
					"name": "Chris Brown",
					"children": [],
					"events": [
						{
							"id": 150100779,
							"name": "Chris Brown & Lil Baby",
							"venueName": "iTHINK Financial Amphitheatre",
							"city": "West Palm Beach",
							"price": 19,
							"distanceFromVenue": 121162.390908814,
							"date": "Aug 12 2022"
						},
						{
							"id": 150096789,
							"name": "Chris Brown & Lil Baby",
							"venueName": "MidFlorida Credit Union Amphitheatre at the Florida State Fairgrounds",
							"city": "Tampa",
							"price": 28,
							"distanceFromValue": 160356.993027791,
							"date": "Aug 13 2022"
						}
					]
				},
				{
					"id": 106819,
					"name": "Daddy Yankee",
					"children": [],
					"events": [
						{
							"id": 10380132,
							"name": "Daddy Yankee",
							"venueName": "The kia Forum",
							"city": "Inglewood",
							"price": 23,
							"distanceFromVenue": 121162.390908814,
							"date": "Aug 16 2022"
						}
					]
				}
			]
		}
	]
}

const getEvents = (node) => {
    const eventsFromChildren = node.children.length !==0 ? node.children.map(getEvents) : [];
    return node.events.concat(...eventsFromChildren)
}

export const events = getEvents(data);